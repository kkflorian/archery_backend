...
public interface UserQuery {
    ...
    @SqlQuery("SELECT * FROM user WHERE (lower(username) like CONCAT('%', lower(:searchTerm), '%') or lower(firstName) like CONCAT('%', lower(:searchTerm), '%') or lower(lastName) like CONCAT('%', lower(:searchTerm), '%')) and id != :userId ORDER BY username LIMIT :limit")
    @RegisterBeanMapper(User.class)
